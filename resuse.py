...
from test_1_tank_controls.retrainingenv import Environment
...
if __name__ == "__main__":
    agent = DQNAgent()
    agent.model.load_weights("models/retrained/sequential__4282.00max_4096.72avg_3925.00min__1576877641.model")
    agent.model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    # For stats
    ep_rewards = [-200]
    ...
    if not os.path.isdir('models/retrained'):
        os.makedirs('models/retrained')
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        ...
        if not episode % AGGREGATE_STATS_EVERY or episode == 1:
            ...
            if average_reward >= MIN_REWARD:
                agent.model.save(
                    f'models/retrained/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.model')

                if(average_reward > best_average_reward):
                    best_model = f'models/retrained/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.model'

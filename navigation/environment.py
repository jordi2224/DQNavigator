...
from navigation import collision
...
class Tank:
    ...
    def get_rays(self):
        ...
        for ray_theta in RAY_THETAS:
            ...
            ray = Wall((self.x, self.y), (end_x, end_y), color=(0, 0, 255))
...
class Waypoint:
    ...
    def update_bb(self):
        self.bounding_box = collision.calculate_bounding_box(self.x, self.y, 0, self.s, self.s)
        for wall in self.bounding_box:
            if self.type == 'goal':
                wall.color = (0, 255, 0)
...
def draw_projections(window, projections):
    ...
    for projection in projections:
        if projection:
            ...
            pygame.draw.rect(window, (0, 0, 255), (projection[0] - s // 2, projection[1] - s // 2, s, s))
...
class Environment:
    ...
    def calculate_reward(self):
        distance_reward = 1 - math.pow(self.to_goal()[0], 0.2)
        angle_reward = 1 - math.pow(math.fabs(self.to_goal()[1]), 0.8)

        return distance_reward * 12 + angle_reward * 5
    ...
    def step(self, action):
        ...
        if (self.to_goal()[0] * cfg.SLS) < cfg.bounding_box_width * 0.7:
            done = True
    ...
    def render(self):
        ...
        # Input rays
        pygame.draw.aaline(self.window, (255, 0, 0), (self.tank.x * cfg.scale, self.tank.y * cfg.scale),
                           (self.hazard.x * cfg.scale, self.hazard.y * cfg.scale))
        pygame.draw.aaline(self.window, (0, 255, 0), (self.tank.x * cfg.scale, self.tank.y * cfg.scale),
                           (self.goal.x * cfg.scale, self.goal.y * cfg.scale))
